...
`serve.py` が launchd 常駐で履歴ページを配っています（`install.sh` が一緒に登録します）。
開くたびに現在の状態から作り直すので、内容は常に最新です。

**「いま確認する」ボタン**を押すとその場で `watcher.py --once` が走り、
終わると自動で表示が更新されます。Mac がスリープしていて記録が古いときに使います。

`history.html` をファイルとして直接開くこともできますが、その場合は
**watcher が最後に実行された時点の内容**で固まっていて、ボタンも押せません
（ブラウザは `file://` のページからコマンドを実行できないため）。
